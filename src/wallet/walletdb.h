...
public:
    ...
    bool WriteZcartSeed(const uint256& hashSeed, const std::vector<unsigned char>& seed);
    bool ReadZcartSeed(const uint256& hashSeed, std::vector<unsigned char>& seed);
    bool ReadZcartSeed_deprecated(uint256& seed);
    bool EraseZcartSeed();
    bool EraseZcartSeed_deprecated();

    bool WriteZcartCount(const uint32_t& nCount);
    bool ReadZcartCount(uint32_t& nCount);
