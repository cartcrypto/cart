// Copyright (c) 2018-2019 The cart developers
// Distributed under the MIT software license, see the accompanying
// file COPYING or http://www.opensource.org/licenses/mit-license.php.

#include <zcart/deterministicmint.h>
#include "zcarttracker.h"
#include "util.h"
#include "sync.h"
#include "main.h"
#include "txdb.h"
...
CzcartTracker::CzcartTracker(std::string strWalletFile)
{
...
CzcartTracker::~CzcartTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzcartTracker::Init()
{
...
bool CzcartTracker::Archive(CMintMeta& meta)
{
...
bool CzcartTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzcartTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzcartTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzcartTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzcartTracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzcartTracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CzcartTracker::GetSerialHashes()
{
...
CAmount CzcartTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzcartTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzcartTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzcartTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzcartTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzcartTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzcartTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzcartTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzcartTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzcartTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzcartTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzcartWallet* zcartWallet)
{
    bool iszcartWalletInitialized = (NULL != zcartWallet);
    CMintMeta meta;
    ...
    if (! iszcartWalletInitialized)
        zcartWallet = new CzcartWallet(strWalletFile);
    meta.isSeedCorrect = zcartWallet->CheckSeed(dMint);
    if (! iszcartWalletInitialized)
        delete zcartWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzcartTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzcartTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzcartTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzcartTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzcartTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzcartTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzcartWallet* zcartWallet = new CzcartWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zcartWallet);
        }
        delete zcartWallet;
        LogPrint("zero", "%s: added %d dzcart from DB\n", __func__, listDeterministicDB.size());
    }
