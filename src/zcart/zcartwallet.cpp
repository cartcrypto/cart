...
CzcartWallet::CzcartWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zcart seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZcartSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZcartSeed_deprecated()) {
                    LogPrintf("%s: Updated zcart seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zcart seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zcart wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzcartWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZcartCount(nCountLastUsed);
    else if (!walletdb.ReadZcartCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzcartWallet::Lock()
{
    seedMaster = 0;
}

void CzcartWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzcartWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZcart(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzcartWallet::LoadMintPoolFromDB()
{
...
void CzcartWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzcartWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzcartWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (std::pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zcartTracker->HasPubcoinHash(pMint.first)) {
...
bool CzcartWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZcart(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zcartTracker which also adds to database
    pwalletMain->zcartTracker->Add(dMint, true);
...
void CzcartWallet::SeedToZcart(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzcartWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzcartWallet::GenerateDeterministicZcart(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzcartWallet::GenerateMint(const uint32_t& nCount, const libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzcartWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzcartWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
