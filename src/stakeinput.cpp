...
CZcartStake::CZcartStake(const libzerocoin::CoinSpend& spend)
{
...
int CZcartStake::GetChecksumHeightFromMint()
{
...
int CZcartStake::GetChecksumHeightFromSpend()
{
    return GetChecksumHeight(nChecksum, denom);
}

uint32_t CZcartStake::GetChecksum()
{
    return nChecksum;
}

// The zcart block index is the first appearance of the accumulator checksum that was used in the spend
// note that this also means when staking that this checksum should be from a block that is beyond 60 minutes old and
// 100 blocks deep.
CBlockIndex* CZcartStake::GetIndexFrom()
{
    if (pindexFrom)
        return pindexFrom;
...
bool CZcartStake::GetModifier(uint64_t& nStakeModifier)
{
    ...
    // zcart staking is disabled long before block v7 (and checkpoint is not included in blocks since v7)
...
CDataStream CZcartStake::GetUniqueness()
{
    //The unique identifier for a zcart is a hash of the serial
    CDataStream ss(SER_GETHASH, 0);
    ss << hashSerial;
    return ss;
}

bool CZcartStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
...
bool CZcartStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal)
{
    //Create an output returning the zcart that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZcartOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zcart output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zcart", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZcartOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zcart output", __func__);
...
bool CZcartStake::GetTxFrom(CTransaction& tx)
{
    return false;
}

bool CZcartStake::MarkSpent(CWallet *pwallet, const uint256& txid)
{
    CzcartTracker* zcartTracker = pwallet->zcartTracker.get();
    CMintMeta meta;
    if (!zcartTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zcartTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}

//!cart Stake
bool CcartStake::SetInput(CTransaction txPrev, unsigned int n)
{
    this->txFrom = txPrev;
    this->nPosition = n;
    return true;
...
bool CcartStake::GetTxFrom(CTransaction& tx)
{
    tx = txFrom;
    return true;
}

bool CcartStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
    txIn = CTxIn(txFrom.GetHash(), nPosition);
    return true;
}

CAmount CcartStake::GetValue()
{
    return txFrom.vout[nPosition].nValue;
}

bool CcartStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal)
{
...
bool CcartStake::GetModifier(uint64_t& nStakeModifier)
{
...
CDataStream CcartStake::GetUniqueness()
{
    //The unique identifier for a cart stake is the outpoint
...
CBlockIndex* CcartStake::GetIndexFrom()
{
    if (pindexFrom)
        return pindexFrom;
    uint256 hashBlock = 0;
