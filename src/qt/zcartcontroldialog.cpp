...
#include "ui_zcartcontroldialog.h"
...
std::set<std::string> ZcartControlDialog::setSelectedMints;
std::set<CMintMeta> ZcartControlDialog::setMints;

bool CZcartControlWidgetItem::operator<(const QTreeWidgetItem &other) const {
    int column = treeWidget()->sortColumn();
    if (column == ZcartControlDialog::COLUMN_DENOMINATION || column == ZcartControlDialog::COLUMN_VERSION || column == ZcartControlDialog::COLUMN_CONFIRMATIONS)
        return data(column, Qt::UserRole).toLongLong() < other.data(column, Qt::UserRole).toLongLong();
    return QTreeWidgetItem::operator<(other);
}


ZcartControlDialog::ZcartControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZcartControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
    ...
    // Title
    ui->labelTitle->setText(tr("Select zcart Denominations to Spend"));
    ui->labelTitle->setProperty("cssClass", "text-title-dialog");


    // Label Style
    ui->labelZcart->setProperty("cssClass", "text-main-purple");
    ui->labelZcart_int->setProperty("cssClass", "text-main-purple");
...
ZcartControlDialog::~ZcartControlDialog()
{
    delete ui;
}

void ZcartControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZcartControlDialog::updateList()
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        CZcartControlWidgetItem* itemDenom(new CZcartControlWidgetItem);
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        CZcartControlWidgetItem *itemMint = new CZcartControlWidgetItem(ui->treeWidget->topLevelItem(mapDenomPosition.at(denom)));
        ...
        {
            LOCK(pwalletMain->zcartTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->zcartTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zcart.";
            else if (!mint.isSeedCorrect)
                strReason = "The zcart seed used to mint this zcart is not the same as currently hold in the wallet";
            else
...
void ZcartControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZcartControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZcart_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    //privacyDialog->setZcartControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZcartControlDialog::GetSelectedMints()
{
...
void ZcartControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
