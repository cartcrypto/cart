...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset){
    if (!settings.contains("nDisplayUnit") || reset)
        settings.setValue("nDisplayUnit", BitcoinUnits::cart);
    ...
    if (settings.contains("nAnonymizecartAmount") || reset)
        SoftSetArg("-anonymizecartamount", settings.value("nAnonymizecartAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in cart.cpp during shotdown to also remove the window positions
