...
#include "qt/cart/send.h"
#include "qt/cart/forms/ui_send.h"
#include "qt/cart/addnewcontactdialog.h"
#include "qt/cart/qtutils.h"
#include "qt/cart/sendchangeaddressdialog.h"
#include "qt/cart/optionbutton.h"
#include "qt/cart/sendconfirmdialog.h"
#include "qt/cart/myaddressrow.h"
#include "qt/cart/guitransactionsutils.h"
...
#include "zcartcontroldialog.h"

SendWidget::SendWidget(cartGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    ...
    btnContacts(new QPushButton())
{
    ...
    /* Button Group */
    ui->pushLeft->setText("cart");
    setCssProperty(ui->pushLeft, "btn-check-left");
    ui->pushLeft->setChecked(true);
    ui->pushRight->setText("zcart");
    setCssProperty(ui->pushRight, "btn-check-right");
    ui->pushRight->setVisible(false);
    ui->pushLeft->setVisible(false);

    /* Subtitle */
    ui->labelSubtitle1->setText(tr("You can transfer public coins (cart)"));
    ...
    /* Address */
    ui->labelSubtitleAddress->setText(tr("Enter a cart address or contact label"));
    ...
    ui->labelAmountSend->setText("0.00 cart");
    ...
    setCssProperty(coinIcon, "coin-icon-cart");
    ...
    // Connect
    connect(ui->pushLeft, &QPushButton::clicked, [this](){oncartSelected(true);});
    connect(ui->pushRight,  &QPushButton::clicked, [this](){oncartSelected(false);});
...
void SendWidget::refreshView(){
    ...
    if(ui->pushLeft->isChecked()){
        btnText = tr("Send cart");
        ui->pushButtonAddRecipient->setVisible(true);
    }else{
        btnText = tr("Send zcart");
...
void SendWidget::refreshAmounts() {
    ...
    bool isZcart = ui->pushRight->isChecked();
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();

    ui->labelAmountSend->setText(GUIUtil::formatBalance(total, nDisplayUnit, isZcart));
    ...
    if (CoinControlDialog::coinControl->HasSelected()){
        ...
        // Wallet's balance
        totalAmount = (isZcart ? walletModel->getZerocoinBalance() : walletModel->getBalance()) - total;
...
void SendWidget::onSendClicked(){
    ...
    bool sendcart = ui->pushLeft->isChecked();

    // request unlock only if was locked or unlocked for mixing:
    // this way we let users unlock by walletpassphrase or by menu
    // and make many transactions while unlocking through this dialog
    // will call relock
    if(!GUIUtil::requestUnlock(walletModel, sendcart ? AskPassphraseDialog::Context::Send_cart : AskPassphraseDialog::Context::Send_zcart, true)){
        // Unlock wallet was cancelled
        inform(tr("Cannot send, wallet locked"));
        return;
    }

    if((sendcart) ? send(recipients) : sendZcart(recipients)) {
...
bool SendWidget::sendZcart(QList<SendCoinsRecipient> recipients){
    if (!walletModel || !walletModel->getOptionsModel())
        return false;

    if(sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        emit message(tr("Spend Zerocoin"), tr("zcart is currently undergoing maintenance."), CClientUIInterface::MSG_ERROR);
        return false;
    }
    ...
    // use mints from zcart selector if applicable
    std::vector<CMintMeta> vMintsToFetch;
    std::vector<CZerocoinMint> vMintsSelected;
    if (!ZcartControlDialog::setSelectedMints.empty()) {
        vMintsToFetch = ZcartControlDialog::GetSelectedMints();
    ...
    if (walletModel->sendZcart(
        ...
        inform(tr("zcart transaction sent!"));
        ZcartControlDialog::setSelectedMints.clear();
        clearAll();
        return true;
    } else {
        QString body;
        if (receipt.GetStatus() == Zcart_SPEND_V1_SEC_LEVEL) {
            body = tr("Version 1 zcart require a security level of 100 to successfully spend.");
        } else {
            int nNeededSpends = receipt.GetNeededSpends(); // Number of spends we would need for this transaction
            const int nMaxSpends = Params().Zerocoin_MaxSpendsPerTransaction(); // Maximum possible spends for one zcart transaction
        ...
        emit message("zcart transaction failed", body, CClientUIInterface::MSG_ERROR);
        return false;
    }
}
...
void SendWidget::onCoinControlClicked(){
    if(iscart){
        if (walletModel->getBalance() > 0) {
            ...
            inform(tr("You don't have any cart to select."));
        }
    }else{
        if (walletModel->getZerocoinBalance() > 0) {
            ZcartControlDialog *zcartControl = new ZcartControlDialog(this);
            zcartControl->setModel(walletModel);
            zcartControl->exec();
            ui->btnCoinControl->setActive(!ZcartControlDialog::setSelectedMints.empty());
            zcartControl->deleteLater();
        } else {
            inform(tr("You don't have any zcart in your balance to select."));
        }
    }
}
...
void SendWidget::oncartSelected(bool _iscart){
    iscart = _iscart;
    setCssProperty(coinIcon, _iscart ? "coin-icon-cart" : "coin-icon-zcart");
    refreshView();
...
void SendWidget::onContactMultiClicked(){
    if(focusedEntry) {
        ...
        CBitcoinAddress cartAdd = CBitcoinAddress(address.toStdString());
        if (walletModel->isMine(cartAdd)) {
        ...
        if (dialog->res) {
            ...
            if (walletModel->updateAddressBookLabels(cartAdd.Get(), dialog->getLabel().toStdString(), "send")) {
