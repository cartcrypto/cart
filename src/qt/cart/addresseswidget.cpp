...
#include "qt/cart/forms/ui_addresseswidget.h"
#include "qt/cart/addresslabelrow.h"
#include "qt/cart/addnewaddressdialog.h"
...
AddressesWidget::AddressesWidget(cartGUI* parent) :
    PWidget(parent),
    ui(new Ui::AddressesWidget)
{
    ...
    // Address
    ui->labelAddress->setText(tr("Enter a cart address"));
...
void AddressesWidget::onStoreContactClicked(){
    if (walletModel) {
        ...
        CBitcoinAddress cartAdd = CBitcoinAddress(address.toUtf8().constData());
        ...
        if (walletModel->updateAddressBookLabels(cartAdd.Get(), label.toUtf8().constData(),
                cartAdd.IsStakingAddress() ? AddressBook::AddressBookPurpose::COLD_STAKING_SEND : AddressBook::AddressBookPurpose::SEND)
                ) {
