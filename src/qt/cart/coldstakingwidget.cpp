...
#include "qt/cart/forms/ui_coldstakingwidget.h"
#include "qt/cart/qtutils.h"
#include "amount.h"
#include "guiutil.h"
#include "qt/cart/requestdialog.h"
#include "qt/cart/tooltipmenu.h"
...
ColdStakingWidget::ColdStakingWidget(cartGUI* parent) :
    PWidget(parent),
    ui(new Ui::ColdStakingWidget)
{
    ...
    /* Subtitle */
    ui->labelSubtitle1->setText(tr("You can delegate your carts and let a hot node (24/7 online node)\nstake in your behalf, keeping the keys in a secure place offline."));
    ...
    ui->labelSubtitle2->setText(tr("Delegate or Accept cart delegation"));
    ...
    ui->btnCoinControl->setSubTitleClassAndText("text-subtitle", "Select cart outputs to delegate.");
...
void ColdStakingWidget::updateStakingTotalLabel()
{
    ...
    ui->labelStakingTotal->setText(tr("Total Staking: %1").arg(
            (total == 0) ? "0.00 cart" : GUIUtil::formatBalance(total, nDisplayUnit))
    );
}
