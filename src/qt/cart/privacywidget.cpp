...
#include "qt/cart/privacywidget.h"
#include "qt/cart/forms/ui_privacywidget.h"
#include "qt/cart/qtutils.h"
#include "guiutil.h"
#include "qt/cart/denomgenerationdialog.h"
#include "qt/cart/txviewholder.h"
...
PrivacyWidget::PrivacyWidget(cartGUI* parent) :
    PWidget(parent),
    ui(new Ui::PrivacyWidget)
{
    ...
    /* Subtitle */
    ui->labelSubtitle1->setText(tr("Minting zcart anonymizes your cart by removing any\ntransaction history, making transactions untraceable "));
    setCssSubtitleScreen(ui->labelSubtitle1);

    ui->labelSubtitle2->setText(tr("Mint new zcart or convert back to cart"));
    ...
    ui->lineEditAmount->setPlaceholderText("0.00 cart ");
    ...
    ui->labelValueDenom1->setText("0x1 = 0 zcart");
    ...
    ui->labelValueDenom5->setText("0x5 = 0 zcart");
    ...
    ui->labelValueDenom10->setText("0x10 = 0 zcart");
    ...
    ui->labelValueDenom50->setText("0x50 = 0 zcart");
    ...
    ui->labelValueDenom100->setText("0x100 = 0 zcart");
    ...
    ui->labelValueDenom500->setText("0x500 = 0 zcart");
    ...
    ui->labelValueDenom1000->setText("0x1000 = 0 zcart");
    ...
    ui->labelValueDenom5000->setText("0x5000 = 0 zcart");
    ...
    // Only Convert to cart enabled.
    ui->containerViewPrivacyChecks->setVisible(false);
    onMintSelected(false);

    ui->btnTotalzcart->setTitleClassAndText("btn-title-grey", "Total 0 zcart");
    ui->btnTotalzcart->setSubTitleClassAndText("text-subtitle", "Show denominations of zcart owned.");
    ui->btnTotalzcart->setRightIconClass("ic-arrow");

    ui->btnCoinControl->setTitleClassAndText("btn-title-grey", "Coin Control");
    ui->btnCoinControl->setSubTitleClassAndText("text-subtitle", "Select cart outputs to mint into zcart.");
    ...
    connect(ui->btnTotalzcart, SIGNAL(clicked()), this, SLOT(onTotalZcartClicked()));
...
void PrivacyWidget::onMintSelected(bool isMint){
    QString btnText;
    if(isMint){
        btnText = tr("Mint zcart");
        ui->btnCoinControl->setVisible(true);
        ui->labelSubtitleAmount->setText(tr("Enter amount of cart to mint into zcart"));
    }else{
        btnText = tr("Convert back to cart");
        ui->btnCoinControl->setVisible(false);
        ui->labelSubtitleAmount->setText(tr("Enter amount of zcart to convert back into cart"));
    }
...
void PrivacyWidget::onTotalZcartClicked(){
    bool isVisible = ui->layoutDenom->isVisible();
    if(!isVisible){
        ui->layoutDenom->setVisible(true);
        ui->btnTotalzcart->setRightIconClass("btn-dropdown", true);
    }else{
        ui->layoutDenom->setVisible(false);
        ui->btnTotalzcart->setRightIconClass("ic-arrow", true);
    }
}
...
void PrivacyWidget::onSendClicked(){
    ...
    if(sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        warn(tr("Zerocoin"), tr("zcart is currently undergoing maintenance"));
        return;
    }
    ...
    if(!GUIUtil::requestUnlock(walletModel, AskPassphraseDialog::Context::Mint_zcart, true)){
        inform(tr("You need to unlock the wallet to be able to %1 zcart").arg(isConvert ? tr("convert") : tr("mint")));
        return;
    }
...
void PrivacyWidget::mint(CAmount value){
    ...
    if(!walletModel->mintCoins(value, CoinControlDialog::coinControl, strError)){
        ...
        // Mint succeed
        inform(tr("zcart minted successfully"));
        // clear
...
void PrivacyWidget::spend(CAmount value){
    ...
    if(!walletModel->convertBackZcart(
            value,
            selectedMints,
            mintChange,
            minimizeChange,
        ...
        // Spend succeed
        inform(tr("zcart converted back to cart"));
        // clear
        ui->lineEditAmount->clear();
    }
}
...
void PrivacyWidget::updateDenomsSupply(){
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zcart </b>";
    ...
    ui->btnTotalzcart->setTitleText(tr("Total %1").arg(GUIUtil::formatBalance(matureZerocoinBalance, nDisplayUnit, true)));
}
