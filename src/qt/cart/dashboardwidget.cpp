...
#include "qt/cart/dashboardwidget.h"
#include "qt/cart/forms/ui_dashboardwidget.h"
#include "qt/cart/sendconfirmdialog.h"
#include "qt/cart/txrow.h"
...
DashboardWidget::DashboardWidget(cartGUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
    ...
    // Staking Information
    ui->labelMessage->setText(tr("Amount of cart staked."));
    setCssSubtitleScreen(ui->labelMessage);
    setCssProperty(ui->labelSquarecart, "square-chart-cart");
    setCssProperty(ui->labelSquarezcart, "square-chart-zcart");
    setCssProperty(ui->labelcart, "text-chart-cart");
    setCssProperty(ui->labelZcart, "text-chart-zcart");
    ui->labelZcart->setVisible(false);
    ui->labelSquarezcart->setVisible(false);
    ...
    ui->labelAmountZcart->setText("0 zcart");
    ui->labelAmountZcart->setVisible(false);
    ui->labelAmountcart->setText("0 cart");
    setCssProperty(ui->labelAmountcart, "text-stake-cart-disable");
    setCssProperty(ui->labelAmountZcart, "text-stake-zcart-disable");
    ...
    ui->btnHowTo->setText(tr("How to get cart"));
...
void DashboardWidget::loadWalletModel(){
    ...
    // update the display unit, to not use the default ("cart")
    updateDisplayUnit();
}
...
const QMap<int, std::pair<qint64, qint64>> DashboardWidget::getAmountBy() {
    ...
    for (int i = 0; i < size; ++i) {
        ...
        bool iscart = modelIndex.data(TransactionTableModel::TypeRole).toInt() != TransactionRecord::StakeZcart;
...
bool DashboardWidget::loadChartData(bool withMonthNames) {
    ...
    chartData->amountsByCache = getAmountBy(); // pair cart, zcart
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            cart = (pair.first != 0) ? pair.first / 100000000 : 0;
            zcart = (pair.second != 0) ? pair.second / 100000000 : 0;
            chartData->totalcart += pair.first;
            chartData->totalZcart += pair.second;
        }

        chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

        chartData->valuescart.append(cart);
        chartData->valueszcart.append(zcart);

        int max = std::max(cart, zcart);
...
void DashboardWidget::onChartRefreshed() {
    ...
    // init sets
    set0 = new QBarSet("cart");
    set1 = new QBarSet("zcart");
    ...
    set0->append(chartData->valuescart);
    set1->append(chartData->valueszcart);

    // Total
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
    if (chartData->totalcart > 0 || chartData->totalZcart > 0) {
        setCssProperty(ui->labelAmountcart, "text-stake-cart");
        setCssProperty(ui->labelAmountZcart, "text-stake-zcart");
    } else {
        setCssProperty(ui->labelAmountcart, "text-stake-cart-disable");
        setCssProperty(ui->labelAmountZcart, "text-stake-zcart-disable");
    }
    forceUpdateStyle({ui->labelAmountcart, ui->labelAmountZcart});
    ui->labelAmountcart->setText(GUIUtil::formatBalance(chartData->totalcart, nDisplayUnit));
    ui->labelAmountZcart->setText(GUIUtil::formatBalance(chartData->totalZcart, nDisplayUnit, true));
