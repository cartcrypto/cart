...
#include "qt/cart/topbar.h"
#include "qt/cart/forms/ui_topbar.h"
#include <QPixmap>
#include "qt/cart/lockunlock.h"
#include "qt/cart/qtutils.h"
#include "qt/cart/receivedialog.h"
...
TopBar::TopBar(cartGUI* _mainWindow, QWidget *parent) :
    ...
    ui(new Ui::TopBar)
{
    ...
    setCssProperty({ui->labelAmountTopcart, ui->labelAmountTopzcart}, "amount-small-topbar");
    setCssProperty({ui->labelAmountcart, ui->labelAmountzcart}, "amount-topbar");
    setCssProperty({ui->labelPendingcart, ui->labelPendingzcart, ui->labelImmaturecart, ui->labelImmaturezcart}, "amount-small-topbar");

    ui->labelTitle2->setVisible(false);
    ui->labelAmountTopzcart->setVisible(false);
    ui->labelAmountzcart->setVisible(false);
    ui->labelTitle5->setVisible(false);
    ui->labelPendingzcart->setVisible(false);
    ui->labelTitle6->setVisible(false);
    ui->labelImmaturezcart->setVisible(false);
...
void TopBar::updateBalances(const CAmount& balance, const CAmount& unconfirmedBalance, const CAmount& immatureBalance,
    ...
    CAmount cartAvailableBalance = balance + delegatedBalance;

    // zcart Balance
    CAmount matureZerocoinBalance = zerocoinBalance - unconfirmedZerocoinBalance - immatureZerocoinBalance;

    // Set
    QString totalcart = GUIUtil::formatBalance(cartAvailableBalance, nDisplayUnit);
    QString totalzcart = GUIUtil::formatBalance(matureZerocoinBalance, nDisplayUnit, true);
    // Top
    ui->labelAmountTopcart->setText(totalcart);
    ui->labelAmountTopzcart->setText(totalzcart);

    // Expanded
    ui->labelAmountcart->setText(totalcart);
    ui->labelAmountzcart->setText(totalzcart);

    ui->labelPendingcart->setText(GUIUtil::formatBalance(unconfirmedBalance, nDisplayUnit));
    ui->labelPendingzcart->setText(GUIUtil::formatBalance(unconfirmedZerocoinBalance, nDisplayUnit, true));

    ui->labelImmaturecart->setText(GUIUtil::formatBalance(immatureBalance, nDisplayUnit));
    ui->labelImmaturezcart->setText(GUIUtil::formatBalance(immatureZerocoinBalance, nDisplayUnit, true));
}
