...
*Setup instructions for a gitian build of cart Coin using a VM or physical system.*

Gitian is the deterministic build process that is used to build the cart
...
to the cart GitHub release page.
...
- [Building cart Coin](#building-cart-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/cart-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/cart-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/cart-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/cart-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/cart-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/cart-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/cartcryptocrypto/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@cartcrypto.io) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/cart-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/cart-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/cart-osx-*-build.assert
```
