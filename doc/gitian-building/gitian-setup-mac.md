...
The build script we'll be using is contained in the cart github repository ([contrib/gitian-build.py](https://github.com/cartcrypto/cart/blob/master/contrib/gitian-build.py)). Since this is a completely fresh environment, we haven't yet cloned the cart repository and will need to fetch this script with the following commands:

```bash
curl -L -O https://raw.githubusercontent.com/cart/cart/master/contrib/gitian-build.py
...
- Clone the gitian-builder, gitian.sigs, cart-detached-sigs, and cart GitHub repos.
